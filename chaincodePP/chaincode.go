...
func (this *IntegrationChaincode) Init(stub shim.ChaincodeStubInterface, function string, args []string) (by []byte, e error) {
	defer func() {
		if err := recover(); err != nil {
			logger.Error("Init panic:", err)
			logger.Error("Init Stack:", string(debug.Stack()))
			switch err.(type) {
			case string:
...
func (this *IntegrationChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) (by []byte, e error) {
	defer func() {
		if err := recover(); err != nil {
			logger.Error("Invoke panic:", err)
			logger.Error("Invoke Stack:", string(debug.Stack()))
			switch err.(type) {
			case string:
				e = errors.New(err.(string))
			case error:
	...
	} else if function == "exchangeCoin" {
		// Transfer
		return this.exchangeCoin(stub, function, args)
	}

	return nil, errors.New("Received unknown function invocation")
}

func (this *IntegrationChaincode) Query(stub shim.ChaincodeStubInterface, function string, args []string) (by []byte, e error) {
	defer func() {
		if err := recover(); err != nil {
			logger.Error("Query panic:", err)
			logger.Error("Query Stack:", string(debug.Stack()))
			switch err.(type) {
			case string:
				e = errors.New(err.(string))
			case error:
